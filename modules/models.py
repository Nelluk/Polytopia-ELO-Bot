...
class Game(BaseModel):
    ...
    cleanup_deferred_until = DateField(null=True, default=None)
    ...
    async def create_game_channels(self, guild_list, guild_id, side=None):
        ...
        for gameside, side_external_server in zip(ordered_side_list, side_external_servers):
            ...
            if (self.notes and 'PCPLUS' in self.notes.upper()) or (self.name and 'PCPLUS' in self.name.upper()):
                # Special case for PolyChamps games with channels locked to a specific server
                    ...
                    for i in range(len(gamesides)):
                        ...
                        for p in side.lineup:
                            ...
                            if self.is_post_moonrise():
                                ...
                                #     logger.info('Skipping local ELO for non-team game (polychampions-specific rule') (reverted March 2022)
                                # else:
    ...
    async def join(self, member, side_arg=None, author_member=None, log_note=''):
        """Compatibility adapter over the shared join application service.

        Command and reaction adapters call the service directly. This method
        remains for older internal callers, but it no longer owns a second
        ...
        try:
            result = await game_join_leave.join(request)
