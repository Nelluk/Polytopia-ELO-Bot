...
async def get_guild_members_by_code(ctx, input):
    # Find matching Guild member(s) by in-game code. Returns a list of discord.Member objects.
    # TODO Might be a tiiiiiiny bit inefficient... feel free to change if needed :)

    matches = []
    
    for p in ctx.guild.members:
        member_id = models.DiscordMember.get_or_none(discord_id=p.id).polytopia_id
        member_mobile = models.DiscordMember.get_or_none(discord_id=p.id).polytopia_name
        member_steam = models.DiscordMember.get_or_none(discord_id=p.id).name_steam
        
        for member_code in (member_id, member_mobile, member_steam):
            if member_code is not None and member_code.lower() == input.lower():
                matches.append(p)
