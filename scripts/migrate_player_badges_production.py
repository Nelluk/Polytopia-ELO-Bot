#!/usr/bin/env python3
"""Plan, verify, or explicitly apply the production player badges migration."""
...
def _offline_plan() -> migration.MigrationPlan:
    return migration.plan_migration(None)
...
def _print_plan(plan: migration.MigrationPlan, *, label: str) -> None:
    ...
    if plan.statements:
        ...
        print('  (already applied and exactly verified)')
    print(
        'rollback disposition: retain the additive column; roll back code '
    ...
    try:
        ...
        if args.verify:
            ...
            if plan.already_applied:
                print('Production player badges schema verification passed read-only.')
                return 0
            print(
                'Production player badges schema verification is incomplete; '
        ...
        print('Production player badges migration transaction committed and verified.')
        return 0
