...
| `delete_player`, `delplayer` | `/operator player delete` | P9.5 audit proposes raw target ID, exact owner-only access, private account-wide orphan graph, fail-closed Lineup/host/bid/API blockers, exact typed confirmation, atomic explicit deletion/audit, and prefix retirement; decisions pending acceptance |
