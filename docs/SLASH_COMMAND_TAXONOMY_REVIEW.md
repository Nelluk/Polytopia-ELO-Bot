...
| `squad`, `squads` | `/squad show` | P7.11 Tier-2 reviewed and integrated: the only invocation is optional integer `squad_id`; omission defaults to requester membership; requester-only one-to-three Discord member selector, paged/selectable snapshot results, and dense card; both prefix registrations retire under C-012. Capability assignment and beta deployment are deferred |
| `squadname` | `/squad name` | P7.12 selected: required squad ID; omission reads, optional name edits, explicit clear removes; authorized `/squad show` cards gain a shared-service Edit Name modal; retire the hidden prefix command under C-013 |
