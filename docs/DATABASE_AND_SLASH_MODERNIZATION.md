...
- P5.1 implementation checkpoints: `b8c7d46`, `fb0ca2b`, `188ba79`,
  `698e775` on
...
- P4.3 implementation/tests checkpoint: `e25e441`; Tier-3 correction
  checkpoint: `bf2275f`; modal-generation correction checkpoint `17a0836` on
...
separate roadmap-evidence checkpoint is `9cb5496`, followed by formatting-only
checkpoint `d0a26c7`. Tier-2 review found no blocking issue; an independent
affected suite passed 55 tests and complete offline discovery passed 854 tests
with 24 intentional gated skips. The branch merged into the accumulation
branch as `36d2605`. The read-only real-schema case, `squad` capability
assignment, command apply, beta restart, announcement, and human acceptance
remain deferred; integration did not disturb the durable beta.
...
gated schema test. Tier-3 review passed and the branch was integrated as merge
...
- `17a0836` — replace the Boolean modal lease with monotonic generations,
  stale-submit rejection, dismissal-without-callback reopening, timeout/error
  invalidation, dispatch-failure recovery, and lifecycle regression coverage.
...
- Draft lookup, validation, permission failures, expiry, and pre-commit
  ...
  retry.
- Compose/Edit uses a monotonic modal generation rather than a Boolean lease.
  Every open captures the next generation; only the newest generation may
  update the draft, and an older submission is rejected privately without
  mutation. Modal timeout/error invalidates only its own generation when still
  current, while Discord dismissal needs no callback before a new Compose/Edit
  click can open. A failed modal dispatch also invalidates its generation, and
  Confirm retains its separate single-flight boundary. GameLog wording records
  that the actor committed a notification request, not that Discord delivery
  succeeded.

Validation evidence:

- Focused P4.3 suite: **21 passed**.
- Affected game/taxonomy/component suites: **115 passed**.
...
- `188ba79` — Preserve legacy prefix warning ordering in committed effect
  snapshots.
...
Branch/base: `codex/p5-4-game-start` from exact clean checkpoint
`4fcdb829139383b800ec3212a1061f8bf54ccb86`.
...
- `258ea13` adds `modules/game_start_workers.py` for the frozen preflight and
  authoritative transition DTOs/workers, `modules/game_start.py` for cached
  Discord-member adaptation and post-commit publication, the direct
  `/game start` registration in `modules/games.py`, and the shared prefix
  service entry point in `modules/matchmaking.py`.
- `tests/test_game_start.py` covers worker-local connections, immutable
  primitive boundaries, authorization/name/pending/full/team/member cases,
  stale-lineup rejection, mutation-family rollback, event-loop heartbeat,
  native defer/visibility, prefix alias/grammar compatibility, and isolated
  post-commit/reconciliation effects. The real-schema transition/rollback is
  covered in `tests/test_database_integration.py`; taxonomy registration is
  covered in `tests/test_slash_taxonomy.py`.
- Focused P5.4 plus taxonomy tests: 19 passed. Existing pending-game and
  slash compatibility suite: 122 passed. Complete offline discovery: 357
  passed, with 16 gated tests skipped. Explicit-venv compilation and
  `git diff --check` passed.
- The selected unchanged safety-gated development integration suite ran 15
  tests successfully, including the UUID-owned real-schema start transition
  and injected GameLog rollback. The gate verified
  `POLYBOT_ENV=development`, database `polytopia_dev`, role `polybot_dev`,
...
- Prefix `$start`/`$startgame` keeps the existing registration and
  bot-channel decorators, exact alias/usage, public failure output, and
  classic `Game.embed`/announcement flow. Native `/game start` defers first,
  performs the same bounded transition, and makes validation/database
  failures ephemeral while committed output remains public.
- After commit, the classic presenter still calls
  `models.Game.load_full_game`; the established
  `Game.update_external_broadcasts`, `Game.create_game_channels`, and
  `league.auto_grad_novas` hooks still use their legacy model-backed
  implementations, and `league.populate_league_team_channels` retains its
  synchronous compatibility refresh. These are explicitly post-commit and
...
- The existing prefix registration decorator remains an inherited
  ...
  worker boundary. A future extraction can move that decorator check without
  changing this unit's transition semantics.
...
Branch/base: `codex/p5-8-open-game-discovery` from exact clean base
`777c697f1e4ca8bf664852065f90d328b60a5f40` on
`codex/database-slash-modernization`.
...
- `/game search query:[optional] view:[optional]` keeps plain invocation on
  **All games** and registers exactly the eight approved typed choices:
  **All games**, **Joinable for me**, **All open**, **Waiting to start**, **My
  open games**, **Active games**, **Completed games**, and **Unconfirmed
  results**. The last choice is rejected by the worker for non-staff and is
  omitted from non-staff Components-v2 controls.
- The existing lifecycle select is replaced by one **Search view** select;
  outcome, common-size, requester-only authorization, cached pagination, and
  public result visibility remain in the same workspace. Native output does
  not expose Mobile/Steam/platform distinctions or extra ranked/size/role/
  member slash arguments.
- `modules/game_search_workers.py` is the shared bounded read/eligibility
  path for native open views and `$games`/aliases. It owns a worker-local
  Peewee connection, accepts a frozen primitive `GameSearchRequest`, applies
  guild/ranked/platform compatibility predicates and a `MAX_GAMES + 1`
  bounded read, and returns frozen row/snapshot DTOs. Requester-aware reads
  cover level, invitation mentions, role locks, capacity, existing
  participant/host inclusion, registered ELO/account eligibility, guild
  isolation, pending/full/expired state, and deterministic waitlist data.
- `$games`, `$opengames`, `$novagames`, and `$nova` retain their existing
...
presentation and the updated `/game search view` option were subsequently
accepted in live smoke testing. No global synchronization, production
operation, push, PR, schema, dependency, or fixture action occurred.
...
`game`, `leaderboard`, and `player`. The separately approved apply inspected
the live guild state, updated only `game`, left `elo`, `leaderboard`, and
`player` unchanged, and created or removed no roots. The tool used explicit
guild scope with no global-sync fallback.

Limitations:

- Open discovery is intentionally capped at the existing bounded DTO limit
...
- The 500-row bounded cap remains the only accepted compatibility limitation;
  beta testing did not make it user-visible.
...
The recommended P5.13 broadcaster action was subsequently completed as
recorded below. The older incomplete-game purge remains policy-gated.
...
- committed roadmap evidence as `b40b20d`, fast-forwarded the clean unit branch
  into `codex/database-slash-modernization`, and pushed that exact checkpoint
  to the approved GitHub origin;
- restarted only `polybot-development-beta@main.service`; its `ExecStartPre`
  writer audit was clear and the sole writer is PID `3171418`, running from the
  clean accumulation checkout at exact checkpoint `b40b20d`;
- the bot authenticated as PolyELO Bot Beta `479029527553638401`, connected to
  Discord, completed startup, and answered the protected local release-control
  status operation;
- made no application-command plan/apply/sync because P5.13 changes no command
  definition or capability;
- selected the explicit no-announcement route because development background
  tasks remain disabled, so the broadcaster does not run and there is no useful
  tester action. Acceptance is owned by the focused and read-only real-schema
  evidence above.
...
The audit found two P5-specific synchronous reads, both resolved by P5.20:
...
capability, or production state changed.

Integration/deployment result:

- fast-forwarded `codex/database-slash-modernization` through code checkpoint
  `a46e99a` and evidence checkpoint `e78601b`, then pushed the accumulation
  branch to the approved GitHub remote;
- restarted only `polybot-development-beta@main.service` from the clean
  accumulation checkout, with no application-command apply/sync;
- authenticated as beta application `479029527553638401`, PID `3267412`, and
  completed the mandatory read-only release-control `status` check;
- sent no tester announcement because the slash tree and primary visible
  workflow are unchanged: this is a prefix-routing implementation detail plus
  retirement of three hidden, explicitly superseded commands; and
- left production, schema, fixtures, command capabilities, and global Discord
  state untouched.

P5 is Complete. Its remaining cross-phase reads are not hidden: P4 owns the
shared immutable post-commit metadata-card presentation cleanup, and P6 owns
the shared asynchronous prefix registration-check cleanup.
...
P5 is **Complete** after P5.20. The two shared seams remain explicitly owned by
the P4/P6 cleanup backlog; they are not silently accepted as completed
...
- add one Analytics section to the existing `/player show` workspace rather
  than adding slash arguments or a second command;
- capture bounded local/global current and all-time rating-history points in
  the initial worker-owned read, then render the selected graph lazily and
  off-loop from immutable data;
- show the requester's confirmed ranked local 1v1 record against a different
  viewed player, preserving the useful legacy follow-up without another
  database query after publication;
- keep section navigation, current/all-time switching, and cached graph reuse
  database-free and requester-bound;
- preserve existing prefix deep links and command permissions;
- keep trophies, favorite-tribe summaries, and pre-Moonrise miscellaneous
  statistics explicitly deferred under C-002.

This is a read-only Tier-2 presentation/database-read unit. It introduces no
schema, mutation, audit, transaction, permission, taxonomy, or command-sync
change. Real-schema validation remains behind the unchanged development gate
and may wait for the next convenient stopped-writer window.

Implementation result:

- Added one Analytics destination to the existing section selector. The slash
  command still has only its optional member and every retained prefix deep
  link keeps its established initial section.
- The worker-owned initial read captures at most the newest 500 local and 500
  global confirmed-ranked history points, including current-reset and all-time
  ELO values, plus one aggregate requester-versus-target local ranked 1v1
  record. Frozen dataclasses are the only worker/view boundary.
- The selected current or all-time graph renders lazily through the bounded
  player executor with an object-owned `Figure`/`FigureCanvasAgg`, immutable
  PNG bytes, and a request-unique attachment name. It uses no pyplot globals,
  shared filename, or filesystem write.
- Each era is cached in the requester-bound public workspace. Era changes and
  repeat navigation do not query PostgreSQL; render/attachment failure remains
  private and leaves the prior public view intact. Cancellation drains the
  submitted renderer before returning.
- Kept the current-reset graph coherent by using Moonrise-era fields rather
  than mixing the archived pre-Moonrise series into it. The all-time graph
  uses the permanent fields. C-002 continues to defer the separate legacy
  trophies, favorite-tribe, and pre-Moonrise miscellaneous presentation.
- Focused player-workspace coverage passed **15 tests**. The affected player,
  leaderboard, taxonomy, and dependency suites passed **48 tests**. Complete
  offline discovery passed **1,159 tests with 42 intentional database-gated
...
Status: **Complete; Tier-2 reviewed and integrated; deployment and beta
acceptance deferred**
...
- P7.11 intentionally retained `$squadname` as a separate write path; P7.12
  now replaces and retires it through `/squad name` without an adapter.
...
- `$squad` and `$squads` registrations were removed without an adapter;
  `$squadname` was retained for the P7.11 checkpoint and is now retired by
  P7.12 without an adapter. `$lbsquad`/`$squadlb` remain unchanged.
...
- Tier-2 review inspected the complete branch, worker ownership, guild and
  channel boundaries, prefix retirement, public/private response behavior,
  requester authorization, expiry, cancellation, and snapshot-only controls.
  No blocking finding required correction.
- Independent affected validation passed 55 tests; complete offline discovery
  passed 854 tests with 24 intentional gated skips. Compilation and branch
  diff checks passed, and the isolated worktree remained clean.
- The complete branch merged into `codex/database-slash-modernization` as
  `36d2605`. This was code integration only: no PostgreSQL access, capability
  assignment, Discord apply/sync, beta lifecycle action, announcement, push,
  production action, dependency install, or sudo occurred.
- Deployment and beta acceptance are intentionally deferred while tester
  activity is low. They can be batched with a later reviewed command unit;
  P7.11 does not need to remain unintegrated while waiting for that session.

Required evidence:

- slash registration and exact optional-integer shape; no `$squad`/`$squads`
  registration; `$squadname` was retained at the P7.11 checkpoint and later
  retired by P7.12;
...
acceptance blocked by P7.11/P7.12 discovery/publication correction**
...
- `modules/squad_identity_workers.py` owns a bounded one-worker ordinary
  write executor, worker-local connection, guild/membership/role-snapshot
  revalidation, explicit whitespace/control validation, the effective
  50-character ceiling, and one synchronous name-plus-`GameLog` atomic
  boundary. `modules/squad_identity.py` is the shared service used by both
  direct `/squad name` mutation and the contextual modal.
- `modules/squad_show_workers.py` adds only frozen requester edit eligibility;
  `modules/squad_show_views.py` and `modules/squad_identity_views.py` render
  the requester-only, expiring Edit name modal. Modal success reloads the
  dense card through the existing bounded squad-show worker and reports a
  committed-but-needs-reconciliation warning if that refresh fails.
- `modules/games.py` registers the exact `/squad name` shape and removes
...
- No production checkout/service/bot/database, push, PR, dependency install,
  or sudo action occurred. Live beta evidence subsequently showed requester
  discovery issuing an unbounded legacy query and never resolving the public
  interaction. Wider-beta squad testing is blocked until the query receives a
...
- tier accepts configured tier names/numbers. A read requires only the legacy
  mod boundary and displays the current value even for an archived or
...
- `modules/administration.py` extends the existing `/team` root with
  `/team name team:[optional] name:[optional]`, `/team server
  team:[optional] server_id:[optional integer] clear:[optional]`, and
  `/team tier team:[optional] tier:[optional configured choice]`. Omitted
  replacements read the current value publicly; committed native changes and
  the approved nullable-server clear identify the actor publicly. Native
...
- `modules/team_attributes_workers.py` owns synchronous lookup, validation,
  ...
  connection and one atomic transaction. Read validation is request-type-safe:
  mutation-only clear checks never access fields absent from read requests.
  Name validation preserves the legacy five-character minimum and composite
...
- `modules/team_emoji_workers.py` now provides the one shared bounded
  autocomplete worker and executor. Suggestions filter by guild, exclude
  hidden/archived teams, apply a hard 25-choice cap, and truncate Discord
  choice display values safely. The same callback is registered on `/team
  emoji`, `/team name`, `/team server`, and `/team tier`; no other guild's
  team rows are returned.
- Tier member-role reconciliation is event-loop-only and post-commit. The
  mutation captures only primitive member IDs before the worker starts; the
  worker performs the persisted Player/preference update, then the event-loop
  reconciliation uses Discord role/member objects to remove managed
  tier/house/League Member and preference roles, restore the committed
  house/tier/League Member roles, and record per-member failures or missing
  managed roles as a public warning without rolling back the database change.
  No Discord await occurs in the worker transaction and no reconciliation runs
  after a worker/database failure. Tier reads intentionally bypass those
  mutation-only house/archive/exact-role checks.
- `tests/test_team_attributes.py`, `tests/test_slash_taxonomy.py`, and the
  retained P8.1 tests cover immutable primitive requests, guild isolation and
  25-choice autocomplete, registration/option shapes, public/private native
  visibility, prefix registration/routing, name minimum/duplicate rollback,
  server integer/read/edit/clear behavior, configured tier lookup and
  house/archive/exact-role preconditions, audit rollback, worker connection
  cleanup, cancellation, event-loop responsiveness, default-deny capability
...
read/edit and approved server clear behavior, name constraints, server-ID
...
and gated development suites, and the combined P8.1/P8.2 beta checklist below.
...
Next action: review the P8.3 correction and documentation checkpoint, then
handle `/team house` separately because its role and
...
workflow`). Review correction implementation/tests checkpoint: `0acdea4`
(`Correct P8.3 team image reconciliation parity`). Documentation/evidence is
intentionally separate from both implementation checkpoints.
...
- preserve `$team_image` registration, required team name, mod plus
  ...
  established prefix success/read wording. The stale lookup example was
  corrected to say `team_image`; there is no native free-form URL option.
...
- database failure leaves the visible local file unchanged and cleans the
  ...
  native output sends the actor-attributed reconciliation warning through the
  established public sender and keeps a staged replacement or hidden recovery
  copy when possible. If public delivery of that committed-state warning
  fails, the same warning is sent ephemerally as a fallback;
- successful reads and committed native mutations are public; native
  validation, permission, lookup/conflict, download, database, and pre-commit
  filesystem failures are ephemeral. Native mutation success identifies the
  actor. The retained prefix keeps its established public compatibility output,
  with committed publication failures using an actor-attributed public
  reconciliation warning.
...
- `modules/administration.py` registers the native command and routes the
  retained prefix command through the shared service. Native committed
  publication failures use the public interaction sender, with an ephemeral
  fallback only when public delivery fails. The existing `team` capability
  remains default-deny; no development capability assignment or registration
  scope was changed.
- `tests/test_team_image.py`, `tests/test_image_storage.py`, and
  ...
  cleanup, successful cancellation drain logging, staged publication ordering,
  unchanged files on DB failure, recoverable publication failure, native public
  reconciliation with ephemeral fallback, and prefix URL/attachment output
  parity through the retained command callback.
- Focused P8 validation was rerun with the development profile and passed **86
  tests** across the P8.1/P8.2 retained suites, image storage, P8.3, slash
  taxonomy, and capability policy. Complete offline discovery passed **625
  tests with 17 intentional integration skips**.
- Oversight independently reran the strict gated development-database suite
  against the verified development profile values `polytopia_dev` and
  `polybot_dev`: **16 tests passed and 1 fixture test was skipped** to preserve
  operator-managed fixtures. No fixture or application data was changed by
...
- a rollback-isolated gated development-database test for the real graph was
  not run. Under the risk-proportional validation cadence, it is recorded for
  the next planned stopped-writer batch because P8.4 extends the established
  team-attribute worker/transaction rather than adding a new writer or schema.
...
- The unchanged gated `tests/test_database_integration.py` gate includes
  a commit-and-audit plus audit-failure rollback test for the real Team and
  GameLog graph, with unique suffix cleanup isolation. After Tier-3 review,
  only the guarded development beta was stopped, a host-wide writer audit
  found no remaining `bot.py --skip_tasks` process, and the suite ran against
  verified `polytopia_dev` as `polybot_dev`: **20 tests passed and 1 existing
  operator-fixture preservation test skipped intentionally**. The new P8.5
  commit/rollback case passed and cleaned up its unique rows.
...
- Independent oversight validation passed **109 affected tests** and the
  complete offline suite at **789 tests with 21 intentional gated skips**;
  compilation, branch diff checks, and a merge simulation passed.
- Integration merge: `37f2a47`. No command synchronization, beta restart,
  Discord inspection, production checkout/service/database action,
  dependency installation, schema change, retained-fixture mutation, push,
  or sudo action occurred. The durable beta remains intentionally stopped
  after the database gate.
...
- the stopped-beta gated development suite passed 29 tests with one
  intentional retained-fixture skip, including the P8.8 real-schema read and
  forced audit-failure rollback case;
- the branch was integrated with checklist checkpoint `6380b19` and pushed;
...
- the guarded beta restarted cleanly at full checkpoint
  `6380b1940e19d1464d74c8a74af3fff2f776616d` and authenticated as PolyELO Bot
...
Risk tier: **Tier 3**. This unit updates an ordinary House token balance and
its audit row but adds no schema and performs no Discord mutation.

Accepted interface, permission, and compatibility decisions:

- `/league tokens house:[optional] amount:[optional integer]
  note:[optional string]` has no required option;
- omission opens all House balances and recent changes; selecting a House or
  supplying `house` opens its balance/history over one immutable bounded
  snapshot;
- reads remain broadly available in the configured league/test guild and do
  not inherit the newer House-management bot-channel restriction;
- supplying `amount` retains the actual legacy `get_user_level > 4` boundary,
  so Helpers/staff level 5 and above may update while level 4 and below may
  only read. This is intentionally not narrowed to Mod-only;
- an optional bounded note is stored in the same actor-attributed audit row;
- `$tokens` is fully retired without an adapter as explicitly approved.

Worker, transaction, and visibility boundaries:

- the event loop captures only primitive guild/requester/level/House/value/
  note/audit identity values before the bounded single-worker path;
- the worker owns its Peewee connection, reloads the House by ID, rechecks
  league scope, level, House name, and expected balance, and rejects stale
  updates rather than overwriting them;
- House balance and actual-guild `GameLog` row share one synchronous
  `db.atomic()` with no Discord await;
- history recognizes both the current `House ID=` audit marker and the older
  `FATS id=` marker, is bounded at 250 rows, and is ordered newest-first by
  timestamp and log ID;
- initial success, loaded-snapshot navigation, and committed updates are
  public and requester-attributed; validation, lookup, permission, database,
  and pre-commit failures remain private;
- a post-commit publication failure is terminal reconciliation and explicitly
  tells staff not to retry the already-committed balance change.

Compatibility C-017 records the full prefix retirement and exact permission
parity.

Implementation/tests checkpoint: `7abfbde`.

Validation evidence:

- focused P8.10 suite: 14 passed;
- affected league/House/policy/taxonomy suites: 60 passed;
- complete offline discovery with asyncio debug timing: 1009 passed with 32
  intentional database-gated skips;
- touched-file compilation and `git diff --check`: passed;
...
- complete offline discovery: **1,084 passed with 36 intentional gated
  skips**;
- touched-file compilation and `git diff --check`: passed;
- after stopping only the guarded development beta and verifying an empty
  host-wide writer audit, the unchanged `development` / `polytopia_dev` /
  `polybot_dev` suite passed **34 tests with one intentional retained-fixture
  skip**. The P8.14 read-isolated case resolved a visible development Team in
  a worker-local connection, mocked only image bytes/fetching, and proved Team
  and GameLog counts unchanged.
...
Status: **Complete on the accumulation branch and deployed; wider-beta acceptance pending**
...
behind the unchanged `development` / `polytopia_dev` / `polybot_dev` gate.
After stopping only the guarded beta and verifying no other development
writer, the suite passed **35 runnable tests with one intentional retained-
fixture skip**. The new P8.15 case read a real registered player and exact
Team and left Player, Team, and GameLog counts unchanged. No production,
dependency, or fixture operation occurred.

The approved read-isolated gate and deployment sequence were then completed.
Live acceptance still requires one exact-role development Team with a stored
image.

Integration/deployment evidence:

- fast-forwarded implementation `e4b7c49`, evidence `2b45d2b`, and database
  evidence `1714473` into `codex/database-slash-modernization`;
- remote inspection planned exactly one update to the existing `league` root,
  with the other nine roots unchanged and no create/removal/global operation;
  the explicit development-guild apply completed and post-inspection converged
  to zero changes;
- the guarded beta restarted at exact clean checkpoint
  `1714473b7308ae36754002dd0d95d0255fb49bca`, authenticated as PolyELO Bot
  Beta `479029527553638401`, and kept background/API/Bullet disabled;
- non-pinging release `2026-08-09-roster-draft` posted once to
  `todo-and-changelog` as message `1535961754859872268`. It states the exact
  Team-role/stored-image prerequisite instead of asking the whole tester pool
  to perform an unavailable showcase test;
- the GitHub push to exact configured origin
  `git@github.com:Nelluk/Polytopia-ELO-Bot.git` was rejected by the environment
  egress reviewer. No workaround was attempted; after this deployment-evidence
  checkpoint the local branch is ahead by four commits pending explicit
  destination approval.

Next action: obtain explicit approval to push the exact GitHub origin, then
configure a stored image on one showcase Team for P8.14/P8.15 acceptance. As
the next code unit, either correct the pre-existing `game_tribe` timing-test/
executor reliability seam so complete discovery is green again, or review the
...
- the development guild already contained the exact unmanaged `Inactive`
  role (`598952755016630289`), so the ignored development-only profile now
  selects that existing role by name; no Discord role was created;
- the private Components v2 workspace pages deterministic oldest-joined
  candidates and exposes Cancel plus one danger-styled refreshed Confirm;
- candidate selection and inactive-role audit writes use dedicated bounded
  worker executors with worker-local Peewee connections; the audit write owns
  one synchronous transaction and no Discord object crosses that boundary;
- Confirm recaptures the guild snapshot and reruns the database query. A
  changed candidate set replaces the preview without Discord effects;
- Discord role changes remain inherently non-atomic, are capped at 100, and
  continue through individual failures. No-success remains retryable and
  private; any success makes later publication failure terminal
  reconciliation rather than inviting duplicate role changes;
- the public no-ping result identifies the actor and aggregate counts only;
  member-level failures/skips remain private; and
- `$deactivate_players` and `$deactivate` are retired under C-022, while
  `$kick_inactive` remains unchanged for P8.19.

Validation evidence:

- focused inactivity/taxonomy/adjacent league suites: **47 passed**;
- complete offline discovery: **1,128 passed with 40 intentional gated
  skips**;
- complete stopped-beta development suite: **39 tests, 38 passed and one
  retained-fixture round trip skipped intentionally** under exact
  `development` / `polytopia_dev` / `polybot_dev` identity;
- the P8.18 real-schema case proved the recent/incomplete-game exclusion,
  candidate selection, worker-local audit commit, and exact audit-row cleanup
  with Player/GameLog/Lineup counts restored; and
- touched-file compilation and `git diff --check`: passed.

Implementation/tests checkpoint: `e61162d`. No schema, persistent fixture,
...
No automatic/scheduled purge is proposed. The user accepted the role policy,
preserved 7/30/60-day thresholds, refreshed preview, typed confirmation,
25-member cap, per-member failure continuation, audit/reconciliation boundary,
and `$kick_inactive` retirement on 2026-08-09. The user also explicitly waived
the prior P8.18 beta-acceptance sequencing gate for this unit.

Implementation result:

- Added Mod-or-owner, league-guild-only
  `/league maintenance kick-inactive` with no slash options. It privately
  defers into a requester-bound Components v2 workspace and evaluates every
  member currently carrying the configured Inactive role.
- Every evaluated member is shown with an eligible or protected reason.
  Selection preserves the effective legacy thresholds: an unregistered member
  must be joined at least 7 days; a registered member must be joined at least
  30 days and have no tracked game in the last 60 days. A pending or incomplete
  league-guild game always blocks removal.
- The worker derives exact Team-role names from current guild-scoped Team
  records. `@everyone`, Inactive, a small explicit starter-role set, and
  current Team roles remain eligible; unknown, managed, Helper/Mod,
  leadership, bot, owner, missing-join-date, and no-longer-Inactive states are
  protected. Worker inputs and results are frozen primitive DTOs.
- Continue opens an exact-count modal. Execution refreshes the complete
  candidate set, requires `KICK <count>`, caps one run at 25, and performs a
  final live role/account check before each Discord effect. DM failure is
  nonfatal; individual kick failure does not abort later candidates.
- Successful Discord removals are recorded afterward in one worker-local
  audit transaction. Audit or public-summary failure is a terminal
  reconciliation result and cannot claim a rollback or make removed members
  retryable. One process-local execution claim plus cancellation draining
  prevents overlapping destructive runs and stranded claims.
- Retired `$kick_inactive` under C-023. No scheduled/automatic removal was
  introduced, Player/DiscordMember history remains retained, and the already-
  deployed `on_member_remove` worker continues to own pending-lineup cleanup.
- New focused P8.19 coverage passed **16 tests**; affected inactivity,
  listener, taxonomy, and gated-module coverage passed **87 tests with 41
  intentional skips**. Complete offline discovery passed **1,155 tests with
  42 intentional gated skips**.
- After stopping only the guarded beta and obtaining a clear host-wide writer
  audit, the unchanged `development` / `polytopia_dev` / `polybot_dev` gate
  ran **41 tests: 40 passed and one operator-fixture round trip skipped
  intentionally**.
- The P8.19 real-schema case proved current Team-role loading, both eligible
  age/registration paths, pending/incomplete blocking, worker-local reads,
...
- remove the `$team_edit` registration and its `TEAM ARCHIVE` grammar without
  an adapter; preserve `$team_tier` as its own retained prefix command and
  continue routing tier changes through the existing attribute worker;
...
- defer slash immediately, keep validation/permission/database failures
  ...
  after commit; and
- add focused permission, confirmation, `$team_edit` retirement and
  `$team_tier` preservation, rollback, connection,
...
This is a slash-schema addition plus an explicitly approved low-use prefix
retirement recorded as C-025. Integration can occur after offline and real-
...
- Git checkpoint lookup is bounded and runs in the storage worker. Cancellation
  drains the worker before returning cancellation; if the drained worker
  committed, it logs only the report ID for reconciliation before re-raising
...
- Confirmed that team archival is virtually never used and does not justify a
  legacy compatibility adapter.
- Updated P8.26 to implement only confirmed Mod-only `/team archive`; retire
  `$team_edit TEAM ARCHIVE` and the `$team_edit` registration completely while
  preserving `$team_tier` as its own worker-backed prefix command.
- Added compatibility-ledger C-025 and retained the prohibition on native
  unarchive without a separate recovery design. This is a documentation-only
  decision checkpoint; no command, database, Discord, beta, or production
  state changed.
...
- Fast-forwarded and pushed the accumulation branch through `e78601b` after
  the complete offline and stopped-beta development PostgreSQL gates passed.
- Restarted only the guarded development beta without application-command
  synchronization; it authenticated as expected application
  `479029527553638401`, PID `3267412`, and the release-control health check
  succeeded.
- Chose no tester announcement because no slash schema or primary visible
  workflow changed. The hidden prefix retirement is already recorded in
  C-024 and `/game side` remains available.
- Marked P5 Complete. The shared live metadata-card presentation seam remains
  assigned to P4, and the cross-module registration decorator remains assigned
  to P6 rather than being silently accepted as P5 debt.
- Next: select the bounded P4 immutable metadata-presentation cleanup so
  side/notes/map/name/tribe post-commit cards no longer reload live Peewee
  models on the event loop.
...
- Subsequently committed roadmap evidence as `b40b20d`, fast-forwarded and
  pushed the accumulation branch, and restarted only the guarded beta. Its
  writer audit was clear, PID `3171418` authenticated as the expected beta
  application, and the protected release-control status operation succeeded.
- Made no command sync or tester announcement because this disabled background
  path exposes no tester-facing action in development.
...
- Added one Analytics section to `/player show` and every retained player-card
  prefix workspace without changing the application-command option shape.
- Captured bounded current/all-time local/global rating points and the
  requester's local ranked 1v1 record in the worker-owned immutable snapshot.
- Added lazy object-owned Agg rendering, unique in-memory attachments, era
  caching, private rollback-safe render failures, and cancellation draining.
- Passed **15** focused tests, **48** affected tests, and complete offline
...
- Accepted the recorded destructive policy and explicitly waived the earlier
  P8.18 beta-acceptance sequencing gate.
- Added a private all-reasons preview, complete-set refresh, exact typed
  confirmation, 25-member cap, current Team-role discovery, fail-closed role
  protection, pending/incomplete-game blocking, per-member Discord failure
  continuation, and post-Discord atomic audit/reconciliation.
- Retired the unsafe immediate `$kick_inactive` path under C-023; retained no
  automatic purge and made no Player/DiscordMember history deletion.
- Passed **16** focused tests, **87** affected tests with **41** gated skips,
  and complete offline discovery with **1,155 passed and 42 intentional
...
- Added Mod-only `/league maintenance mark-inactive` with a private bounded
  preview, refreshed confirmation, deterministic 100-member action cap, and
  public actor-attributed no-ping aggregate.
- Preserved the legacy 60-day current-guild activity and incomplete-game
  rules while explicitly excluding bots, the bot owner, already-inactive,
  recent-join, and protected-role members.
- Moved the inactive-role branch of `on_member_update` into a dedicated
  worker-local audit transaction and retained the unrelated listener paths.
- Retired `$deactivate_players` and `$deactivate` under C-022; retained
  `$kick_inactive` unchanged for P8.19.
- Selected the development guild's existing unmanaged `Inactive` role
  (`598952755016630289`) through ignored development-only configuration; no
  Discord role was created.
- Focused suites passed **47**; complete offline discovery passed **1,128
  with 40 intentional skips**; the stopped-beta development suite ran **39
  tests: 38 passed and one retained-fixture round trip skipped intentionally**.
- The real-schema P8.18 case cleaned up its exact audit row and restored all
...
- Recorded implementation/tests checkpoint `e4b7c49`; database, integration,
  command apply, restart, and wider-beta acceptance initially remained
  pending. The approved stopped-writer gate then passed **35 runnable tests
  with one retained-fixture skip**, including the read-isolated P8.15 case;
  integration and deployment remain next.
- Fast-forwarded P8.15 into the accumulation branch, updated only the existing
  development-guild `league` root, verified post-apply convergence, and
  restarted the guarded beta at `1714473b` with the expected identity.
- Posted non-pinging release `2026-08-09-roster-draft` as message
  `1535961754859872268`, accurately naming the stored-image prerequisite.
- The exact GitHub-origin push was blocked by the environment egress reviewer;
  after this evidence checkpoint the branch remains locally ahead by four
  commits and no bypass was tried.
...
- The guarded beta and PostgreSQL were untouched during implementation. The
  approved gate later stopped only the beta, found no other development
...
- Added `/league tokens` with optional House, amount, and note inputs under
  the reserved default-deny `league` capability.
- Preserved exact legacy permission behavior: broad reads in the league/test
  guild and level-5-or-higher updates, without adding a bot-channel gate or
  silently narrowing Helpers to Mod-only.
- Added one requester-bound Components v2 snapshot for all balances, recent
  changes, House-specific history, paging, and House selection; controls do
  not requery PostgreSQL.
- Moved balance mutation and actor-attributed audit into one worker-local
  synchronous transaction with expected-balance conflict protection,
  cancellation draining, post-commit publication, and terminal
  reconciliation after a committed publication failure.
- Retired `$tokens` without an adapter as approved and recorded C-017.
- Validation: 14 focused P8.10 tests, 60 affected league/House/policy/taxonomy
  tests, and 1009 complete offline tests passed with 32 intentional database
  skips. Compilation and diff checks passed. The real-schema gate remains for
  the approved stopped-beta integration window.
- Implementation checkpoint: `7abfbde` from exact base `2816500`.
...
- Fast-forwarded implementation `c86d604` and evidence `b5b710c` into the
  accumulation branch, added the operator-owned checklist, and pushed clean
  checkpoint `6380b19` before stopping the beta.
- Stopped only `polybot-development-beta@main.service`; the host-wide guard
  found no remaining development beta writer. The unchanged development gate
  passed 29 tests with one intentional operator-fixture skip, including the
  new House read and forced audit-failure rollback case against
  `polytopia_dev` as `polybot_dev`.
- The explicit guild-only command apply updated only the existing `house`
  root in guild `478571892832206869`; post-apply inspection found all nine
  roots unchanged and no create, removal, or global operation.
- Restarted the guarded beta at full checkpoint `6380b1940e19d1464d74c8a74af3fff2f776616d`,
  verified PolyELO Bot Beta `479029527553638401`, and passed release-control
  status.
- Posted tester-pinged **WHAT TO TEST** release
  `2026-08-08-house-attributes` once to `todo-and-changelog` as message
  `1535804396816957540`. No production or global operation occurred.
...
- A real `/squad show` requester-discovery invocation at 15:09:49 reached the
  ...
  loop remained responsive and other commands continued, so the failure is
  isolated to the bounded read/publication path rather than global bot
  blocking.
- Live query evidence showed `_bounded_query_rows()` using a Python slice on
...
- Implemented exact `/squad name squad_id name:[optional] clear:[optional]`
  public reads and member-or-staff edits/clears on isolated branch
  `codex/p7-12-squad-identity`, based on
  `ec347cad25f8bcaa059ee6c1d54e7744b63ef8f8`; code/test commit is `f04d017`.
- Added the shared frozen-DTO identity service and bounded worker-local
  name-plus-audit transaction, including guild/membership/staff revalidation,
  rollback coverage, explicit normalization/clear semantics, actor
  attribution, and cancellation/concurrency responsiveness evidence.
- Added captured member/staff Edit name visibility to `/squad show`; modal and
  direct slash mutation use the same service, and modal commits refresh the
  dense card through the bounded squad-show path with committed-warning
  reconciliation when refresh fails.
- Retired `$squadname` completely without an adapter and recorded C-013. The
  existing squad-show and squad-leaderboard eligibility, ordering, paging,
  record/rank, recent-game, channel, and `allow_teams` behavior remains
  unchanged.
- Focused identity/show/taxonomy suites passed 20/18/8 tests; complete
  offline discovery passed 875 tests with 25 intentional gated skips;
  compileall and diff checks passed. The gated real-schema commit/rollback
  test was added but not run while the durable beta is active. Capability
  assignment, command sync, beta, production, database/fixture access, and
  `docs/BETA_WHAT_TO_TEST.md` changes remain deferred.
- Next action: Tier-3 review and later accumulation integration in a separate
  reviewed step; no active beta monitoring is expected from this unit.
...
- Verified exact ancestry and reviewed the complete squad adapter, worker,
  Components workspace, permission/visibility behavior, prefix retirement,
  tests, and evidence. No blocking finding required a correction.
- Independently passed 55 affected tests and the complete 854-test offline
  suite with 24 intentional gated skips; compilation and diff checks passed.
- Integrated the complete branch as accumulation merge `36d2605`.
- Deferred the read-only real-schema gate, `squad` capability assignment,
  Discord command apply, beta restart, announcement, and human acceptance so
  they can be batched into a later active testing session. The durable beta
  was not disturbed.
- Next action: select another bounded offline unit; `/squad name` is the
  smallest coherent continuation, subject to an explicit `$squadname`
  retention/retirement decision before implementation.
...
- Independently reviewed the complete P8.5 branch and found no code blocker.
  Affected validation passed **109 tests**; complete offline discovery passed
  **789 tests with 21 intentional gated skips**; compilation and diff checks
  passed.
- Stopped only `polybot-development-beta@main.service`, confirmed it inactive,
  and required the host-wide development-writer audit to report no remaining
  `bot.py --skip_tasks` process before PostgreSQL access.
- Ran the unchanged explicit `development` / `polytopia_dev` / `polybot_dev`
  gate: **20 tests passed and 1 existing operator-fixture preservation test
  skipped intentionally**. The new Team+GameLog commit/audit/rollback case
  passed with unique cleanup isolation.
- Integrated implementation `eafe219` and evidence `4fa81bc` as merge
  `37f2a47`, preserving the newer beta follow-up/reporter-notification process
  checkpoint. The durable beta remains stopped; no command apply/sync,
  restart, announcement, production, dependency, push, or sudo action
  occurred.
- Next action: prepare and review the exact development-guild command plan,
  then separately approve apply/restart and wider-beta smoke/announcement
  disposition for `/team create`.
...
### 2026-08-03 — WB1.1 staffhelp compatibility correction implemented

- Corrected the development-only prefix store gate so production
  `$staffhelp`/`$helpstaff` continues its established relay, optional card,
  `GameLog`, and public acknowledgement path without beta capture/store calls.
- In development, unsupported, over-limit, or unreadable prefix attachments
  now retain their legacy URLs and JSONL draft while omitting local bytes; the
  native upload limits remain strict. Missing-channel and relay failures after
  commit report the opaque ID without resetting cooldown or claiming legacy
  success. Native followup-send failures and cancellation after a committed
  worker result log body-free report IDs for reconciliation.
- Correction implementation/tests checkpoint: `9e8f899`. Focused validation
  passed **46 tests**; complete offline discovery passed **645 tests with 17
  intentional integration skips**. The unchanged gated development-database
  evidence remains valid because the correction does not change database
...
- Added the typed attachment/read/clear native shape under the existing
  ...
  success/read wording. Native public success identifies the actor; native
  failures remain private except for committed publication reconciliation.
- Added worker-local effective-image reads and atomic audited URL-state
  ...
  introduced.
- The initial focused P8 validation passed with **80 tests**. The initial
  complete offline discovery passed with **619 tests and 17 intentional
  integration skips**. The later correction rerun and current evidence are
  recorded below.
...
### 2026-08-03 — P8.3 review correction completed locally

- Corrected native committed-publication failure handling in `0acdea4`:
  the actor-attributed reconciliation warning now goes through the established
  public interaction sender, and only a public-delivery failure triggers the
  same warning as an ephemeral fallback.
- Added adapter-level retained `$team_image` tests for direct URL replacement
  and attachment-plus-URL attachment-wins behavior, including established
  output and shared-service routing. Corrected the stale lookup example from
  `team_emoji` to `team_image` without changing the legacy success/read output.
- Added a successful caller-cancellation regression proving the worker
  transaction and filesystem publication drain to completion, the staged file
  is cleaned up, and the committed/publication outcome is logged without any
  Discord send from the cancelled path.
- Reran focused P8 validation: **86 tests passed**. Reran complete offline
  discovery: **625 tests passed with 17 intentional integration skips**.
- Oversight independently reran the strict development-database suite against
  `polytopia_dev`/`polybot_dev`: **16 passed and 1 fixture test skipped** to
  preserve operator-managed fixtures. No command synchronization, Discord
  inspection, runtime/beta launch, production action, dependency installation,
  push, merge, or sudo action occurred.
...
### 2026-08-03 — P8.2 implementation checkpoint completed locally

- Implemented `/team name`, `/team server`, and `/team tier` on branch
  `codex/p8-2-team-simple-attributes` from exact clean base `eab8388`.
  The implementation/tests checkpoint is commit `de8a0a9`.
  Extended the existing team root's shared autocomplete to all four focused
  attributes, retained default-deny capability policy, and preserved the
  `$team_name`, `$team_server`, `$team_tier`, `$team_house`, and archive paths.
- Focused validation passed with **48 tests**, including the retained P8.1
  team-emoji/policy coverage. Complete offline discovery passed with **598
  tests and 17 skips** under the development profile.
- The unchanged development database gate was invoked with
  `POLYBOT_ENV=development POLYBOT_RUN_DB_INTEGRATION=1` against the verified
  `polytopia_dev`/`polybot_dev` setup. Its PostgreSQL connection was
  unavailable, so setup failed before any integration test ran; no fixture,
  application data, or production resource was changed.
- No capability assignment, Discord inspection/synchronization, beta launch,
  production checkout/service/database action, dependency installation, push,
  merge, or sudo action was performed. The combined P8.1/P8.2 beta checklist
  remains pending separate approval.

### 2026-08-03 — P8.2 Tier-3 review corrections completed locally
...
- Captured only primitive exact-team-role member IDs before the worker;
  ...
  audit failure rolls the full change back; Discord role reconciliation remains
  post-commit and reports partial failures publicly.
- Added five focused regressions. Focused P8.1/P8.2 validation passed with 53
...
- Applied the reviewed command tree only to development guild
  `478571892832206869` using the explicit deployment tool and confirmation
  gates. Discord updated `game`, left `elo`, `leaderboard`, and `player`
  unchanged, and created or removed no roots; no global synchronization path
  was available or used.
- Launched one development beta from integrated checkpoint `6bbd147` with
  startup synchronization disabled and confirmed beta application
  `479029527553638401` authenticated successfully.
- The user accepted `/game search` open-game discovery, view switching, and
  retained prefix behavior without reporting defects. The task-owned beta was
  then stopped cleanly.
- P5.8 is Complete and beta-validated. No production operation, push, PR,
  fixture, schema, dependency, or global command action occurred.
...
- Created `codex/p5-8-open-game-discovery` from exact clean base
  `777c697f1e4ca8bf664852065f90d328b60a5f40` and committed implementation/
  tests as `34f385c`. The primary planning checkout and production checkout
  were not accessed or edited.
- Added the typed optional `/game search view` initial selection and one
  coherent Components-v2 Search view control. Plain search remains All games;
  Joinable for me, All open, Waiting to start, My open games, Active,
  Completed, and staff-only Unconfirmed map through the shared immutable
  worker snapshot. Outcome, size, pagination, public output, and
  requester-only controls remain available.
- Moved open-game query, requester eligibility, bounded waitlist reads, and
  DTO presentation data into the existing two-worker game-search executor.
  Prefix `$games`, `$opengames`, `$novagames`, and `$nova` now use that path
  while retaining their legacy mode, alias, ranked/unranked, channel, and
  Steam parsing behavior. No mutation service was called from browsing.
- Added focused coverage for eligibility restrictions, invitations, role
  locks, capacity, membership/host inclusion, expiration, guild isolation,
  exact registration/mapping, staff denial, controls/defer/pagination,
  prefix parity, worker connection ownership, bounded concurrency, and slow
  reads. A temporary in-memory runtime-profile harness passed 42 tests.
- The prescribed runtime-profile focused/full/gated commands remain
  unavailable in this worktree because `config.development.ini` is absent;
  they were not weakened and no database identity or fixture operation was
  established. `py_compile` and `git diff --check` passed.
...
### 2026-08-01 — P5.4 implemented locally

- Implemented the two-stage primitive preflight/member-snapshot/final-worker
  boundary and committed the complete code/test checkpoint as `258ea13` on
  `codex/p5-4-game-start`, based on exact `4fcdb829139383b800ec3212a1061f8bf54ccb86`.
- Preserved `$start`/`$startgame` grammar, aliases, registration/channel
  checks, public prefix behavior, and classic card density while adding the
  deferred typed `/game start game_id name` with ephemeral native failures.
- Verified atomic team/squad/side/game/league/audit mutation, stale-lineup
  protection, shared pending-game worker ownership, and post-commit
  announcement/card/channel/league/Nova effect isolation.
- Passed 19 focused P5.4/taxonomy tests, 122 pending-game compatibility tests,
  357 complete offline tests with 16 gated skips, compilation, and diff
  checks. The selected safety-gated development suite passed 15 tests under
  `development`/`polytopia_dev`/`polybot_dev`, including UUID-owned start
  transition and rollback coverage; no operator fixture was touched.
- No beta launch, Discord inspection/apply/sync, production operation, or
  live smoke occurred. Status remains Implemented locally pending Tier-3 Sol
  review; the next action is separate Sol review followed by any approved
...
- Implemented and committed P5.1 on `codex/p5-1-game-open` as `b8c7d46`, with
  audit-note and warning-order parity corrections in `fb0ca2b` and
  `188ba79`,
...
- Added offline fault, ordering, concurrency, responsiveness, registration,
  ...
  rolls back the complete pending-game graph. The initial offline suite
  passed 274 tests with 13 gated skips; the initial gated attempt was blocked
  by unavailable PostgreSQL at `localhost:5432`.
- No beta launch, Discord inspection/synchronization, production operation,
  dependency change, merge, or push occurred.

### 2026-07-31 — P5.1 native parity correction

- Corrected `/game open` to inherit the configured unranked-channel default
  while preserving the requester ranked/unranked toggle.
- Added an explicit primitive cross-play platform-validation mode. Native
  requests retain canonical `is_mobile=True` storage and accept either mobile
  or Steam account names; neither-name requests receive canonical account
  registration guidance. Prefix requests retain exact Mobile/Steam checks.
- Added focused regressions for native channel defaults, Steam-only success,
