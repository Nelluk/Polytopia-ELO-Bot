...
The preparation source permits an exact production Discord snapshot and an
offline import plan. Production import remains deliberately disabled until the
runtime, operator workers, all five table schemas, and rollback tooling are
promoted together in a later reviewed source unit.
...
1. Complete and review the production-authority source unit. It must promote
   the exact production target in startup and owner/delegated operator workers,
   create/verify the registry, revision, audit, draft, and delegation tables,
   use production-specific initial-import audit identity, and retain fail-closed
   startup with no static fallback when database authority is selected.
2. Merge the reviewed source and deploy it while production remains on
...
3. Capture one bounded live Discord role/channel snapshot and produce the
...
4. Under separate database-write approval, take the normal production backup,
   atomically create all five additive tables, import the 49 revision-one
   documents, and verify every row and digest. The running bot can remain on
   static authority because this version does not read those tables.
5. Inspect the exact per-guild Discord command diff from the imported bundle.
...
6. In one short approved maintenance action: stop the production bot, verify
   zero production writers, change the bound `config.ini` selector to
   `database`, apply the reviewed guild command plans, and start the bot.
7. Verify the authenticated identity, all 49 published runtime documents,
