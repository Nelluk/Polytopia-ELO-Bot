...
class polygames(commands.Cog):
    ...
    @commands.command(aliases=['user'], usage='player_name')
    async def getuser(self, ctx, *, code_string: str | None = None):
        """Get username of a player from their code
        Just returns the username and nothing else so it can easily be copied."""

        if code_string is None:
            return await ctx.send(f'You did not input a polytopia code.\n**Usage:** `{ctx.prefix}getuser PLAYER NAME HERE`')
        
        code_string_safe = discord.utils.escape_mentions(code_string)

        guild_matches = await utilities.get_guild_members_by_code(ctx, code_string)

        if len(guild_matches) == 0:
            return await ctx.send(f'Could not find any server member matching *{code_string_safe}*.')

        elif len(guild_matches) > 1:
            return await ctx.send(f'Found {len(guild_matches)} server members matching *{code_string_safe}*:\n{"\n".join(['`' + m.name + '`' for m in guild_matches])}')
        
        return await ctx.send(f'`{guild_matches[0].name}`')
