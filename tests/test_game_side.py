...
class GameSidePrefixTests(unittest.IsolatedAsyncioTestCase):
    def command(self):
        return next(
            command
            for command in matchmaking.matchmaking.__cog_commands__
            if command.name == 'gameside'
        )

    def context(self, *, author_id=100, role_mentions=(), invoked_with='gameside'):
        ...
        return SimpleNamespace(
            ...
            prefix='$',
            invoked_with=invoked_with,
            message=SimpleNamespace(role_mentions=list(role_mentions)),
            send=mock.AsyncMock(),
        )

    @staticmethod
    def game(*, host_id=100, pending=True):
        harness = SideHarness(host_id=host_id, pending=pending)
        return harness.game

    async def test_prefix_aliases_preserve_role_and_success_output(self):
        command = self.command()
        self.assertEqual(command.aliases, ['matchside', 'sidename'])
        role = SimpleNamespace(
            id=77,
            name='Ronin',
            guild=SimpleNamespace(id=300),
        ...
        ctx = self.context(
            role_mentions=(role,),
            invoked_with='matchside',
        )
        seen = {}
        result = game_workers.GameSideMutationResult(
            42, 300, 401, 1, None, 'Ronin', None, 77, 'Ronin',
            False, False,
        )
        ...
        async def publish(committed, **kwargs):
            await kwargs['send'](game_side.legacy_mutation_message(committed))
        ...
        with mock.patch.object(
            ...
            await command.callback(
                ...
                self.game(),
                '1',
                args='Ignored by role lock',
            )

        request = seen['request']
        self.assertEqual(request.side_lookup, '1')
        self.assertEqual(request.role_id, 77)
        self.assertEqual(request.role_name, 'Ronin')
        self.assertIsNone(request.side_name)
        self.assertFalse(request.native)
        self.assertEqual(request.invoked_with, 'matchside')
        ctx.send.assert_awaited_once_with(
            'Side 1 for game 42 has been locked to role **@Ronin** and '
            'named **Ronin**'
        )

    async def test_prefix_none_clear_and_permission_checks_stay_in_place(self):
        command = self.command()
        ctx = self.context(invoked_with='sidename')
        seen = {}
        ...
        self.assertTrue(seen['request'].clear)
        self.assertIsNone(seen['request'].side_name)

        denied_ctx = self.context(author_id=999)
        ...
        denied_ctx.send.assert_awaited_once_with(
